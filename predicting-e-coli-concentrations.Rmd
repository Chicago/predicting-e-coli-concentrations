...
We included various measurements of wind since `r citet(biblio["smith_effect_1999"])` and `r citet(bilbio["olyphant_elements_2004"])` found that wind is a reliable predictor of FIB levels. In particular, beaches downwind from point-pollution sources or where FIB was active are more likely to experience elevated _E. coli_ levels. We used the Dark Sky API to determine the wind direction and intensity each day, as well has historical data preceeding a beach sample.
