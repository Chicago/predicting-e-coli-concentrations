...
Prior-day Nowcast models used simultaneous measures of the previous day _E. coli_ levels. Limited deployments of qPCR testing, however, means some beach data could be available before other culture-based testing are completed. We use available qPCR data to denote "feature beaches" to use _E. coli_ readings to predict bacteria levels at other beaches based on past correlation.

![Chicago Beaches often fluctuate with each other. The dotted line represents the logarithm of the typical beach advisory decision threshold (235 CFU/100ml)](img/beach-relationship.png)

We use selected qPCR readings ($x_i^t$) from today $t$ at feature beaches, that is $X^t: x_1^t, x_2^t, ..., x_n^t$, to forecast bacteria levels ($y_i^t$) at other ($m$) beaches that do not have any _E. coli_ readings: $Y^t: y_1^t, y_2^t, ..., y_m^t$. The number of feature beaches ($n$) does not need to equal the number of predicted beaches ($m$). Only one feature beach is needed to predict one or more beaches in the same cluster, that is, $n \leq m$. 

## Identifying Beach Clusters

Given a number of desired clusters ($k$), the objective is to group feature beaches ($x_i^t$) with a cluster of other beaches ($y_i^t$) together that are correlated and have similar characteristics. The result is $k$ number of clusters with memberships of $k: \{ x_{1 \in k}^t, y_{1 \in k}^t, y_{2 \in k}^t, ..., y_{m \in k}^t \}$. Each cluster consists of the feature beach, $x_{i \in k}^t$, to predict the values at the other beaches in the cluster, $y_{i \in k}^t$.

We used K-means near clustering algorithm to detect when beaches have similar movements. First, there is an initial, random guess for which beaches belong in clusters. The distance or error is then measured for all clusters. Then, membership of clusters is slightly altered, the error is then measured and determined if the error is increasing or decreasing. This process is repeated until the lowest measured error is obtained.
...
A random forest regression model was trained with the predicted beach's name and 10 years of culture-based test results for each of the feature beaches. The model was tuned and validated using leave-one-year-out cross validation, in which the validation set consisted of all the observations in one year, and the training set consisted of all other observations. The process was repeated until each year had been in the validation set. The year 2016 was left out as a final validation set.
...
Table 2: Hybrid model 2017 pilot results.

Over the summer, the analytical model had a true positive rate of 11.2 percent compared to a true positive rate of 3.2 percent in 2016 (Table 2). From 2015 through 2016, the historical average true positive rate for those beaches was 3.3 percent. That is, the hybrid model increased the true positive rate by 250 percent over the historical average. 
...
Table 3: Comparing specificity, sensitivity, and accuracy between Hybrid and Prior-day Nowcast Models.
...
The difference in performance is likely due to the use of bacteria levels on the same day. Chicago beaches rarely encounter consecutive days of elevated bacteria levels, so prior-day readings do not predict elevated levels on the next day. 

This approach can be valuable for recreational beach systems which have several beaches. In particular, it is potentially valuable when newer qPCR methods cannot be installed at all beaches. While qPCR methods can reliably deliver same-day results and preclude the need for predictive models, the relatively higher cost, complexity and expertise may limit widespread deployment across all beaches.

Not all beach systems may exhibit correlation between beaches over time. Each recreational beach network would need to explore the historical correlation between beaches and determine if there are any beach clusters. In Chicago, correlation between beaches decreased with distance between beaches and some beaches, because of physical features, were not strongly correlated with other beaches.

# Acknowledgements

We are indebted to Chicago Park District staff, including Cathy Breitenbach and Carol Kim, who provided incredible guidance and feedback while taking time to work with the research team. Meredith Nevers garciously provided the research team with her considerable expertise that was invaluable. We are also grateful for all of the volunteers from ChiHackNight who helped conduct some of the analysis. Finally, we wish to thank the attendees of State of Lake Michigan conference in 2017 for their questions and comments.
