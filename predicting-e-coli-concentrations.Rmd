...
source("Master.R")
hybrid <- model
source("Master-alternate.R")
multivariate <- model
source("Master-alternate-2.R")
multivariateAltered <- model
model <- hybrid
...
Table A.2 shows results comparing the multivariate model to the Hybrid model discussed in the body of the paper. The Area Under Curve (AUC), a general measurement for perfomance, was slightly higher (0.846) than the model which only relied on interbeach correlation (0.833). While the multivariate model technically performs better, the insignificance of the difference indicates that the addition of the additional variables does not increase predictive power.

We also tested an altered form of the model that removed the same-day test results as a predictor, instead it only used the variables in Table 1 as the predictor. The AUC dropped signifcantly to 0.701, which shows a significant loss in accuracy.
