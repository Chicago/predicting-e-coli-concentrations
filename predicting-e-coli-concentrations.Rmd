...
```{r variable_importance, echo=FALSE, fig.height=6, fig.width=6, results='hide', message=FALSE, warning=FALSE}
varImp <- importance(model, main = "", type=1)
varImp <- data.frame("Beach" = row.names(varImp),
                     "Percent_Increase_MSE" = varImp[,1])
varImp$Beach <- factor(varImp$Beach, levels = varImp$Beach[order(varImp$Percent_Increase_MSE)])
p <- ggplot(varImp) + 
  geom_col(aes(Beach, Percent_Increase_MSE)) 
grid.arrange((p + scale_fill_Publication() + theme_Publication()) +
...
```{r ROC, echo=FALSE, fig.height=6, fig.width=6, results='hide', message=FALSE, warning=FALSE}
## build USGS ROC
...
p <- ggplot() + 
  geom_line(aes(x = model_summary$fpr, y = model_summary$tpr, color = "Hybrid Model")) + 
...
grid.arrange((p + theme_Publication()),nrow=1)
```
...
```{r pilot_results, echo=FALSE, fig.height=6, fig.width=6, results='hide', message=FALSE, warning=FALSE}
# 2017 Pilot results
...
```{r model_comparison, echo=FALSE, fig.height=6, fig.width=6, results='hide', message=FALSE, warning=FALSE}
plotDf2 <- data.frame("Model" = c("2017 Hybrid", "2016 Prior-day", "2015 Prior-day"),
...
grid.table(plotDf2, theme=gridTheme, rows = NULL) 
