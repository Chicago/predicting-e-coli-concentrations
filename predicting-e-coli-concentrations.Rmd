...
Chicago Park District measures water quality of 20 beaches along 26 miles against Lake Michigan on Chicago's eastern shore. FIB in Lake Michigan are introduced through multiple mechanisms and rarely from single-point pollution events. Foreshore and submerged beaches are an important contributor to FIB. People and animal activity on foreshore sands can introduce bacteria on beaches, which can be introduced to the water through rainfall, waves, and through swash zones (Whitman, 2003).

Wave activity can transport FIB to beaches and encourage resuspesion (Ge, 2010). Moreover, wet sand can act as a repository for FIB, storing it between seasons and outside of cyclical patterns (Alm, 2003). FIB can be (re-)introduced to the water at a later time, allowing for FIB concentrations to be introduced outside of cyclicals that can be explained by weather and hydrometeorological conditions.

The effect of waves can be accentuated when beaches have a U-shape where FIB can be resuspended or trapped in swash zones (Ge, 2012). Likewise, breakwaters contribute to the existence of FIB in some beaches. They interfere with the flow of water, trapping it in some areas, while also interrupting wind and wave patterns (Whitman, 2001). The water near a breakwater wall is also slightly warmer and can encourage FIB growth.

Birds are also a significant source of contamination at beaches (Levesque, 2000; Haack, 2003; Alm, 2018). Gulls, in particular, are often associated with FIB concentrations in beaches. Contamination can occur from birds flying over the water. Likewise, contamination can occur on the beach and be introduced to the water through the aforementioned mechanisms.
...
The Chicago River is contiguous to Lake Michigan but a lock limits water flowing into the lake. The City of Chicago has a combined sewer system which, during excess rain events, must redirect excess mass into the Chicago River. The river is contiguous to Lake Michigan but a lock limits water flowing into the lake. However, the lock is occassionally opened to release water from the Chicago River into Lake Michigan when excess water accumulates in the river. While the lock is opened--generally less than once per year--beaches are immediately closed `r citep(biblio["chicago_metropolitan_water_reclamation_district_reversals_2016"])`.
