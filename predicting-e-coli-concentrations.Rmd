...
`r citer(biblio[c("ackerman_relationship_2003", "morrison_receiver_2003")])` found that large rainstorms increased FIB levels 2 to 5 days after the rain has ceased. The research team collected data from the Dark Sky application programmer interface (API) to understand forecasted rainfall for a day of water measurement and also understand various historical measures of rainfall.

Likewise, the same data source was used to determine the amount of sunlight the lake was exposed to at the beaches. `r citet(bilio["whitman_solar_2004"])` found that increased sunlight inactived _E. coli_ at a Chicago beach, whereas cloudy days decreased _E. coli_ found. Similar to precipitation, Dark Sky data was used to measure forecasted cloud cover as well as historical measurements of cloud cover preceding water quality tests.

We included various measurements of wind since `r citet(biblio[c("smith_effect_1999", "olyphant_elements_2004")])` found that wind is a reliable predictor of FIB levels. In particular, beaches downwind from point-pollution sources or where FIB was actived are more likely to experience elevated _E. coli_ levels. We used the Dark Sky API to determine the wind direction and intensity each day, as well has historical data preceeding a beach sample.

Similarly, wave and tidal patterns seem to influence observed FIB levels. `r citet(bilio["le_fevre_role_2003"])` found that _E. coli_ levels could be resuspended under tidal waves, allowing _E. coli_ to be observed in locations only reached by waves or tidal levels. In a multivariate model, `r citet(bilio["crowther_relationships_2001"])` showed that tidal levels affected observed FIB levels. Unfortunately, our team did not have access to suffucient historical data to determine wave and tidal levels, but we used lunar phases to approximate the 1-day and 3-day average lake level. [NICK, PLEASE ADD TO THE LAKE LEVELS IF NEEDED]
...
Variables <- c("Forecast for today", "Yesterday total rainfall", "3 day total rainfall", "Total rainfall total until 8am",
               ...
               "1-day tidal levels", "3-day tidal levels", "Tidal levels at 8am",
               "1-day lake level", "3-day average lake level",
...
knitr::kable(Variables, caption = "Variables used for multivariate Hybrid model", escape = FALSE) %>% 
  ...
  kableExtra::group_rows("Tidal Levels", 14, 16) %>%
  kableExtra::group_rows("Lake Level", 17, 18) %>%
  kableExtra::group_rows("Visitor Density", 19, 20)
  
```

Table A.2 shows results comparing the multivariate model to the Hybrid model discussed in the body of the paper. The Area Under Curve (AUC), a general measurement for perfomance, was slightly lower (0.829) than the model which only relied on interbeach correlation (0.832). While the difference is not numerically large, it indicates that the addition of the additional variables does not increase predictive power.

We also tested an altered form of the model that removed the qPCR test results as a predictor, instead it only used the variables in Table 1 as the predictor. The AUC dropped signifcantly to 0.627, which shows a significant loss in accuracy.

[INSERT TABLE OF MODEL RESULTS]
