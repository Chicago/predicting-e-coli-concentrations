...
Overall, the hybrid model correctly raised warnings by 230 percent. When a beach was predicted to have an elevated level, it is 61.7% percent less likely to have been a "false positive" (type I error). The increase in correct predictions, however, was not induced by simply raising more erroneous warnings. The "false negative" rate only slightly increased (type II errors) by two-tenths of a percent.
...
We explored the addition of other predictors and covariates, but it did not increase the model performance over the model only using interbeach correlation. We suspect that any information and predictive-power provided by covariates have already been captured by the qPCR testing conducted at other beaches. For instance, rainfall can approximate the conditions for FIB growth, the variation of FIB levels at other beaches is a better predictor since it's reliant on actual beach conditions. The temporal linkage between observations for ideal bacteria conditions and actual bacteria growth has been hard to demonstrate in a large ecosystem like Lake Michigan. Meanwhile, the interbeach correlations seems to provide a better signal that ideal conditions already exist. Nothwithstanding, we also realize our hydometrological data is based on third-party sources instead of active sensors in the water. Other predictors on visitor density are very loose approximations based on available data. These could be improved in the future and may provide better capability for prediction.
