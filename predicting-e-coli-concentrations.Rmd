...
Recently, researchers have had an expanding toolkit of machine learning algorithms to formulate predictions `r citep(biblio["brooks_predicting_2016"])`. These algorithms can be swapped with $f(...)$ to forecast bacteria levels at beaches. In this section, we will propose various different "prior-day" models which use prior culture-based tests to predict whether bacteria levels exceed the acceptable threshold.
...
Authors have repeatedly noted the one-day lag in data negatively impacts the ability for prior-day models to accurately predict for beach advisories and closure. The correlation between the prior-day bacteria readings has little correlation with elevated bacteria levels in the current period.
