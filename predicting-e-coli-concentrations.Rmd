...
The precision of the model--the portion of predicted high FIB that were accurate compared to all predicted FIB beach days--also grew from 17 to 27 percent--a 59 percent increase. Meanwhile, the false positive rate was slightly higher at 2.0 percent during 2017. Area Under Curve (AUC) is a measure that shows the ability of the model to show the trade-off between the true positive rate and false positive rate. The AUC for the hybrid model is 0.837, which is higher than the prior-day model that was around 0.64 in 2015 and 2016. A higher AUC does indicate the model has improved the ability to estimate beaches with elevated bacteria levels without raising a disproportionate number of false positives, but there does appear to be significant room for improvements in the model performance.

We also tested a model that used a combination of interbeach correlation with predictors for precipitation, sunlight, wind, proxies for human density, and tidal and lake levels. The performance of the multivariate hybrid model was similar to the hybrid model without other predictors. AUC for the multivate hybrid model was 0.855, 0.018 higher. This indicated that our predictors only provided marginal improvements to the precision of the predictions, where most of the predictive power is provided by the interbeach correlation.

To test the predictive power of the hydrometerological covariates, we also formulated another multivariate model which eliminates the interbeach correlation. The altered multivariate model is a random forest model using the predictors discussed in section 2.5. That model performed much lower than either the hybrid or multivariate hybrid model with an AUC of 0.703.
